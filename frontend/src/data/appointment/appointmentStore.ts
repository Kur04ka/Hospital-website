...
export const useAppointmentStore = create<AppointmentStore>((set, get) => ({
	...
	createAppointment: async (appointment_id: number) => {
		try {
			const response = await instance.post<Appointment>(
				`/appointments/create-new-appointment/${appointment_id}`,
				{},
				{
					headers: {
	...
	fetchCurrentAppointments: async () => {
		try {
			const response = await instance.get<Appointment[]>(
				"/appointments/current-appointments-by-uuid",
				{
					headers: {
						Authorization: `Bearer ${localStorage.getItem("jwt")}`,
					},
	...
	fetchArchiveAppointments: async () => {
		try {
			const response = await instance.get<Appointment[]>(
				"/appointments/archieve-appointments-by-uuid",
				{
					headers: {
						Authorization: `Bearer ${localStorage.getItem("jwt")}`,
					},
	...
	updateCurrentAppointment: async (
		old_appointment_id: number,
		new_appointment_id: number,
	) => {
		try {
			...
			set({
				currentAppointments: currentAppointments.map(appointment =>
					appointment.appointment_id === old_appointment_id
						? response.data
						: appointment,
				),
			});
