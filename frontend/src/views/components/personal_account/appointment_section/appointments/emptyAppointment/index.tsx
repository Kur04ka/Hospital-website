...
const EmptyAppointment: React.FC<EmptyAppointmentProps> = ({handleMakeAppointmentFormClick}: EmptyAppointmentProps) => {
    return (
        <Box display={'flex'} gap={'3rem'} justifyContent={'center'} textAlign={'center'} padding={'3rem'} border={'3px solid rgba(238, 243, 248, 1)'} borderRadius={'3rem'}>
            ...
            <Box display={'flex'} flexDirection={'column'} justifyContent={'center'} gap={'1rem'}>
                ...
                <Button fullWidth className={styles.button} onClick={handleMakeAppointmentFormClick} variant='contained'>Записаться на прием</Button>
            </Box>
        </Box>
    )
};
