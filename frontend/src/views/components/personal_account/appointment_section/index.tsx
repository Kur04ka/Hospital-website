...
const AppointmentSection = () => {
	...
	const [selectedType, setSelectedType] = useState<"current" | "archive">(
		"current",
	);
	const [hasArchiveAppointments, setHasArchiveAppointments] =
		useState<boolean>(false);
	const [hasCurrentAppointments, setHasCurrentAppointments] =
		useState<boolean>(false);
		...
		>
			<Box display={"flex"} justifyContent={"space-between"}>
				...
				<Button
					...
					onClick={handleMakeAppointmentFormClick}
					variant="contained"
				>
					Записаться на прием
				</Button>
				...
				{selectedType === "current" ? (
					...
					appointments.map(appointment => (
						<Box
							...
							borderRadius={4}
							padding={"1rem"}
							sx={{ backgroundColor: "rgba(238, 243, 248, 1)" }}
						>
							<Typography
								variant="h6"
								color={"rgba(137, 155, 181, 1)"}
			...
			<EditDialog
				...
				old_appointmentId={selectedAppointmentId}
				doctorName={selectedDoctor}
			/>
			<MakeAppointmentForm
				open={isMakeAppointmentOpen}
				setOpen={setMakeAppointmentOpen}
			/>
		</Box>
	);
};
